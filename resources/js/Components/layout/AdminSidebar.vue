<script setup>
import { LayoutDashboard, FileText, Image as ImageIcon, BarChart3, Users, Settings, LogOut, PlusCircle, X, ChevronDown } from 'lucide-vue-next'
...
<template>
    <aside :class="[isOpen ? 'w-fit lg:w-64 translate-x-0' : 'w-20 -translate-x-full lg:translate-x-0']"
        ...
        <div class="flex items-center justify-between h-20 px-6 border-b border-gray-50 dark:border-gray-800 shrink-0">
            <Link href="/" class="flex items-center gap-3">
            <div class="flex items-center justify-center w-8 h-8 font-black text-white bg-red-700 shrink-0">
                {{ appName1 ? appName1.split(' ')[0][0] : 'T' }}
            </div>
            <span class="text-lg font-black tracking-tighter uppercase transition-all duration-300 dark:text-white"
                v-if="isOpen">
            ...
            <div v-for="item in menuItems" :key="item.name">
                <div v-if="item.children">
                    <button @click="toggleMenu(item.name)"
                        class="flex items-center justify-between w-full p-3 transition-all outline-none group" :class="[
                            isChildActive(item.children) || expandedMenus.includes(item.name)
                                ? 'text-red-700 bg-gray-50 dark:bg-gray-800/50 border-l-4 border-red-700 '
                                : 'text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800 hover:text-gray-900 dark:hover:text-white',
                            isOpen ? 'rounded-lg' : 'justify-center'
                        ]">
                        ...
                        <ChevronDown v-if="isOpen" class="w-4 h-4 transition-transform duration-300"
                            :class="[expandedMenus.includes(item.name) ? 'rotate-180' : '']" />
                    </button>

                    <div v-if="isOpen && expandedMenus.includes(item.name)"
                        class="mt-2 ml-6 space-y-1 border-l-2 border-gray-100 dark:border-gray-800">
                ...
                <Link v-else :href="item.href" class="flex items-center p-3 transition-all group" :class="[
                    $page.url === item.href
                        ? 'bg-gray-50 dark:bg-gray-800 text-red-700 border-l-4 border-red-700 rounded-l-lg'
                        : 'text-gray-400 border-transparent hover:bg-gray-50 dark:hover:bg-gray-800 hover:text-gray-900 dark:hover:text-white',
                    isOpen ? 'rounded-l-lg' : 'justify-center'
                ]">
                <component :is="item.icon" class="w-5 h-5 shrink-0" />
        ...
        <div class="p-4 border-t border-gray-50 dark:border-gray-800 shrink-0">
            <button @click="logout" :class="[!isOpen && 'justify-center']"
                class="flex items-center w-full p-3 text-gray-400 transition-colors cursor-pointer hover:text-red-700 group">
