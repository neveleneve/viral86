...
const menuItems = [
    ...
    },
    {
        ...
        icon: BarChart3,
        children: [
            { name: 'Iklan Aktif', href: '/admin/iklan-aktif' },
            { name: 'Permintaan Iklan', href: '/admin/permintaan-iklan' },
        ],
    },
    {
        name: 'Pengguna',
        icon: Users,
        children: [
            { name: 'Redaksi', href: '/admin/redaksi' },
            { name: 'Pembaca', href: '/admin/pembaca' },
            { name: 'Pengiklan', href: '/admin/pengiklan' },
        ],
    },
    ...
    <aside :class="[
        ...
        <nav class="flex-1 p-4 space-y-2 overflow-y-auto custom-scrollbar">
            <div v-for="item in menuItems" :key="item.name">
                ...
                <Link v-else :href="item.href" class="flex items-center p-3 transition-all group rounded-xl" :class="[
                    ...
                    <component :is="item.icon" class="w-5 h-5 shrink-0" />
                    <span class="ml-4 text-[10px] font-black uppercase tracking-[0.2em] whitespace-nowrap"
                        v-if="isOpen">
                        {{ item.name }}
                    </span>
                </Link>
        ...
        <div class="p-4 border-t border-gray-50 dark:border-gray-800 shrink-0">
            <button @click="logout" :class="[!isOpen && 'justify-center']"
                class="flex items-center w-full p-3 text-gray-400 transition-colors cursor-pointer rounded-xl hover:bg-red-50 dark:hover:bg-red-950/30 hover:text-red-700 group">
